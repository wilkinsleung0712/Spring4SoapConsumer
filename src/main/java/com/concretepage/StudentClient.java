...
public class StudentClient extends WebServiceGatewaySupport {
	public GetStudentResponse getStudentById(int studentId){
		...
		GetStudentResponse response = (GetStudentResponse) this.getWebServiceTemplate().marshalSendAndReceive(request, new SoapActionCallback("http://localhost:8080/Spring4SoapProducer/soapws/getStudentResponse"));
		return response;
	}
}
